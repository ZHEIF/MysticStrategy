...
def load_api_key() -> str:
    ...
    for candidate in candidates:
        ...
        raw = read_text_with_fallback(candidate)
...
def deepseek_config() -> Dict[str, Any]:
    ...
    timeout = int(os.getenv("DEEPSEEK_TIMEOUT_SECONDS", "60") or 60)
...
def fallback_self_analysis(payload: Dict[str, Any]) -> Dict[str, Any]:
    name = clamp_text(payload.get("name"), 60) or "你"
    birth_place = clamp_text(payload.get("birth_place"), 80) or "出生地"
    plans = clamp_text(payload.get("recent_plans"), 200)
    experiences = clamp_text(payload.get("major_experiences"), 200)
    summary = f"{name} 的重点是把近期规划从发散信息收束成清晰节奏，尤其要先稳住执行，再谈扩张。"
    if plans:
        summary = f"{name} 当前的主线更像是在 {plans[:40]} 这类目标上做收束。"

    def block(title: str, text: str) -> Dict[str, Any]:
        return {
            "trend": text,
            "advantages": [
                f"把 {title} 拆成周计划，优先完成最可见的一步。",
                "把外部反馈当作校准信号，而不是否定信号。",
            ],
            "risks": [
                "目标过多时容易同时开太多口子。",
    ...
    return {
        ...
        "overview": {
            "headline": f"{name} 的总基调是先定方向，再定节奏。",
            "core_archetype": f"更适合把 {birth_place} 这类环境信息转成稳定的行动锚点。",
            "key_strengths": [
                "信息吸收快，适合做整合型判断。",
                "一旦节奏固定，执行力会明显提升。",
            ],
        ...
        "self_analysis": {
            "nature": f"你给出的经历更像是 {experiences[:80] or '阶段性变化较多'}，这会让你对趋势很敏感，但也更需要有节奏地筛选信息。",
            "pattern": "玄学模型上的共同指向是：先收敛，再放大；先布局，再加速。",
            "supporting_models": [
                {"model": "奇门遁甲", "read": "先找门，再找局，不要一上来就全面铺开。"},
                {"model": "五行六运", "read": "当前更适合做补短板和补节奏，而不是一味追强刺激。"},
                {"model": "幸福数字密码", "read": "把高频重复动作固定下来，比一次性爆发更有收益。"},
                {"model": "占星术", "read": "更需要可复盘的轨道，而不是完全靠临场灵感。"},
            ],
        },
        "next_90_days": {
            "career": block("事业/工作", "未来三个月的工作面更适合低噪音推进，先把最能见效的任务打穿。"),
            "study": block("学业/认知", "未来三个月的认知提升来自持续输入和定期输出，而不是大量囤积。"),
            "family": block("家庭", "家庭面宜保持低摩擦沟通，先稳情绪，再谈分工和支持。"),
            "love": block("爱情", "感情面更看重真实、稳定和一致性，急推进反而容易失衡。"),
            "relationships": block("人际关系", "人际面适合先筛选高质量互动，再把资源给值得长期协作的人。"),
            "health": block("健康", "身体面要把作息、运动和饮食做成固定流程，不要靠意志硬撑。"),
        ...
        "next_3_years": {
            "year_1": {
                "theme": "先搭骨架",
                "career": "工作上要先建立稳定产出结构。",
                "study": "认知上建立方法论和复盘习惯。",
                "family": "家庭关系以稳定沟通为主。",
                "love": "感情重在识别真正适配的人。",
                "relationships": "人际关系先做圈层筛选。",
                "health": "健康重在作息和节奏。",
                "wealth": "财务重在止损和储备。",
            },
            "year_2": {
                "theme": "再扩边界",
                "career": "在稳定主业上再增加第二增长点。",
                "study": "把已学会的方法外化成工具或流程。",
                "family": "关系边界更清楚，协作更顺。",
                "love": "关系进入更现实的检验期。",
                "relationships": "合作关系开始分层。",
                "health": "身体管理更系统。",
                "wealth": "现金流开始更重视结构优化。",
            },
            "year_3": {
                ...
                "study": "认知模型开始稳定输出。",
                "family": "家庭支持方式更成熟。",
                "love": "更适合长期、稳定、互相尊重的连接。",
                "relationships": "人际网络趋向少而精。",
                "health": "如果前三年打好了基础，健康会更可控。",
                "wealth": "财富更依赖长期纪律而不是短线波动。",
            },
            "cross_year_trends": [
                "主线会越来越清晰，但前提是你愿意删掉次要噪音。",
                "每次阶段变化，都要先复盘再出手。",
            ],
        },
        "high_leverage_moves": [
            "把最近最重要的 1-2 个目标写成周度行动清单。",
            "每周固定一次复盘，记录有效动作和失误点。",
            "先建立稳定的睡眠、运动和工作节奏。",
        ],
        "risk_controls": [
            "任何超过三条的并行目标都要重新排序。",
            "遇到焦虑时先减速，不要在情绪最高点做重大决定。",
        ],
        "closing": "以上是基于你提供信息的结构化兜底分析；若 DeepSeek 可用，会得到更细的分层推演。",
    }


def fallback_friend_analysis(payload: Dict[str, Any]) -> Dict[str, Any]:
    people_raw = payload.get("people")
    people = []
    if isinstance(people_raw, list):
        for item in people_raw[:12]:
            if not isinstance(item, dict):
                continue
            people.append(fallback_friend_person_analysis(item, payload))
    ...
    return {
        ...
        "closing": "以上是基于输入的安全兜底分析；DeepSeek 可用时会给出更细的人物分层与节奏建议。",
    }


def fallback_friend_person_analysis(person: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    name = clamp_text(person.get("name"), 60) or "某位朋友"
    tendency = clamp_text(person.get("relationship_tendency"), 160)
    plans = clamp_text(person.get("recent_plans"), 160)
    return {
        "name": name,
        "profile": f"{name} 当前更像是在 {plans[:60] or '推进自己的目标'}，关系上适合用公开、低压、可持续的方式相处。",
        "dynamics": f"你对 {name} 的关系倾向是“{tendency[:60] or '保持清晰方向'}”，建议把关系定位成透明协作，而不是隐性操控。",
        "symbolic_reading": [
            "奇门遁甲：先看场，再看门，别急着上强度。",
            "五行六运：先补稳态，再谈放大。",
...
def analyze_friend_person(payload: Dict[str, Any], person: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = fallback_friend_person_analysis(normalized_person, payload)
        result["warning"] = clamp_text(exc, 300)
        return result
...
def aggregate_friend_analysis(payload: Dict[str, Any], people: List[Dict[str, Any]], person_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    warning = next((item.get("warning") for item in person_results if item.get("warning")), "")
    return {
        "ok": True,
        "source": "deepseek" if any(item.get("warning") is None for item in person_results) else "local-fallback",
        "model": deepseek_config()["model"] if any(item.get("warning") is None for item in person_results) else "local-heuristic",
        "analysis": {
            "source": "deepseek" if any(item.get("warning") is None for item in person_results) else "local-fallback",
        ...
        "timestamp": now_iso(),
        "warning": clamp_text(warning, 300) if warning else "",
    }
...
def analyze(mode: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    if mode == "self":
        ...
        try:
            ...
            return {
                ...
                "analysis": fallback_self_analysis(normalized_payload),
                "timestamp": now_iso(),
                "warning": clamp_text(exc, 300),
            }
    ...
    if not person_results:
        person_results = [fallback_friend_person_analysis(person, normalized_payload) for person in people]
