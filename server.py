...
class AccessCodeRequiredError(RuntimeError):
    pass


class AccessCodeInvalidError(RuntimeError):
    pass
...
def require_access_code(headers: Optional[Any], payload: Dict[str, Any]) -> None:
    ...
    if not provided:
        raise AccessCodeRequiredError("需要访问码才能调用 DeepSeek")
    if not hmac.compare_digest(provided, config["access_code"]):
        raise AccessCodeInvalidError("访问码不正确")
...
def reserve_deepseek_units(client_ip: str, units: int) -> None:
    ...
    with RATE_LIMIT_LOCK:
        ...
        if daily_units + units > config["max_calls_per_day"]:
            ...
            raise DeepSeekRateLimitError(f"今日 DeepSeek 调用额度已用完，剩余 {remaining} 个单位")
        if ip_hour_units + units > config["max_calls_per_ip_per_hour"]:
            remaining = max(0, config["max_calls_per_ip_per_hour"] - ip_hour_units)
            raise DeepSeekRateLimitError(f"当前 IP 调用过于频繁，请稍后再试，剩余 {remaining} 个单位")
...
def analysis_error_response(exc: Exception) -> tuple[Dict[str, Any], HTTPStatus]:
    if isinstance(exc, AccessCodeRequiredError):
        return {"ok": False, "code": "access_code_required", "error": str(exc)}, HTTPStatus.UNAUTHORIZED
    if isinstance(exc, AccessCodeInvalidError):
        return {"ok": False, "code": "access_code_invalid", "error": str(exc)}, HTTPStatus.UNAUTHORIZED
    if isinstance(exc, DeepSeekRateLimitError):
        return {"ok": False, "code": "rate_limited", "error": str(exc)}, HTTPStatus.TOO_MANY_REQUESTS
    if isinstance(exc, ValueError):
        return {"ok": False, "code": "invalid_request", "error": str(exc)}, HTTPStatus.BAD_REQUEST
    return {
        "ok": False,
        "source": "deepseek",
        "error": f"DeepSeek 深度思考请求失败：{exc}",
